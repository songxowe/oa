$(function() {
	...
	var worldMapContainer = document.getElementById('main2');
	
	// 用于使chart自适应高度和宽度,通过窗体高宽计算容器高宽
	/*var resizeWorldMapContainer = function() {
		worldMapContainer.style.width = window.innerWidth * 0.60 + 'px';
		//worldMapContainer.style.height = window.innerHeight * 0.6 + 'px';
		worldMapContainer.style.height =$("#div2").css("height");
	};*/
	// 设置容器高宽
	...
	$.ajax({
				...
				success : function(data) {
					...
					if (data.json) {
						...
						if(data.mark==1){
							var st = "<input type='button' id='goVote2' value='选择一个投票' />";
							sum += st;
							$("#myform2").append(sum);
						}
						
						$("#goVote2").click(function(){
							$("#myform2").form('submit');
						});
