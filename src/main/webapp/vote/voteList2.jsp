...
<body>
	<div class="easyui-layout" fit="true">
		<div data-options="region:'north',title:'高级查询'"
			...
			主题名:<input type="text" class="easyui-textbox" placeholder="输入主题名" id="subject"> <label>创建日期:</label>
			<input id="beginDate2" class="easyui-datetimebox">~ <input
				id="endDate2" class="easyui-datetimebox"><input type="hidden"
				id="qwer2"> <input type="hidden" value="${param.voteStatus}">
			<a href="#" class="easyui-linkbutton" iconCls="icon-search"
				onclick="advPubVote2()">查询</a>
		</div>
		<div data-options="region:'center'">

			<table id="dgVote"></table>
			<div id="voteView"></div>
		</div>
	</div>
	<div id="dv1"></div>
	<script type="text/javascript">
  	...
  	$(function(){
  		...
  		$("#dgVote").datagrid({
  			...
  			rownumbers:true,
  			columns:[[
  			  {field:'fromId','title':'编号',width:80,sortable:true,hidden:true},
  	          ...
  	          {field:"op",title:"操作",width:220,
  		    	...
  		    	formatter:function(value,rowData,rowIndex){
  		    		...
  		    		var button="<button onclick=\"findVote("+voteId+")\">详细信息</button><button onclick=\"showVote2("+voteId+")\">图表信息</button>";
  		    		var userId=${sessionScope['NEWER_USER_LOGIN_INFO'].id };
  		    		if(fromId==userId){
  		    			button +="<button onclick=\"updateStatus('"+subject+"',"+voteId+")\">终止投票</button>";
  		    		}
  		    		return button;
  		    		//return "<a href=\"#\" class=\"easyui-linkbutton\" iconCls=\"icon-search\" onclick=\"findVote("+voteId+")\" plain=\"true\">详细信息</a><button onclick=\"showVote2("+voteId+")\">图表信息</button><button onclick=\"updateStatus('"+subject+"',"+voteId+")\">终止投票</button>";
...
//  		});
  	...
  	*/
  	...
  	//修改状态
  	function updateStatus(subject,voteId){
  		//var subject="1234";
  		$.messager.confirm('提示信息','你要终止'+subject+'投票吗?',function(flag){
  			if(flag){
  				$.ajax({
  					url:"updateVote",
  					//join方法，将数组每个元素按定义的分隔符拼接成字符串
  					...
  					beforeSend:function(){
  						//进入数据加载状态
  						$("#dgVote").datagrid("loading");
  					},
  					success:function(data){
  						//结束加载状态
  						$("#dgVote").datagrid("loaded");
  						//加载第一页数据
  						$("#dgVote").datagrid("load");
  						$.messager.show({
  							title:"提示信息",
  	...
  	function showVote2(id){
		...
		document.getElementById("voteView").value=result; */
		...
		//alert(id);
  		...
  		$("#voteView").window({
  			title:"查看详细信息",
  			width:800,
  			height:600,//window.innerHeight * 0.6
  			modal:true,
  			minimizable:false,
